document.addEventListener('DOMContentLoaded', () => {
    let userPermissions = { // Default permissions for demonstration
        "shutdown": true,
        "restart": true,
        "lock": true,
        "play_pause": true,
    ...
    const closeButtons = document.querySelectorAll('.close-button, .close-button-nested, .close-button-alert');
    ...
    document.querySelector('.action-button.restart').addEventListener('click', () => {
        showNotification('Reboot command sent (simulated).', 'success');
    });

    document.querySelector('.action-button.lock').addEventListener('click', () => {
        showNotification('Lock session command sent (simulated).', 'success');
    });

    // Multimedia Control Buttons
    document.querySelector('.action-button.media-previous').addEventListener('click', () => {
        showNotification('Previous track command sent (simulated).', 'info');
    });

    document.querySelector('.action-button.play-pause').addEventListener('click', () => {
        showNotification('Play/Pause command sent (simulated).', 'info');
    });

    document.querySelector('.action-button.media-next').addEventListener('click', () => {
    ...
    volumeMuteButton.addEventListener('click', () => {
        ...
        showNotification(`Volume ${isCurrentlyMuted ? 'unmuted' : 'muted'} (simulated).`, 'info');
    });
    ...
    function updateSystemMetrics() {
        ...
        // Simulate uptime
        const now = new Date();
        const bootTime = new Date(now.getTime() - (Math.random() * 86400000 * 7 + 3600000)); // Up to 7 days + 1 hour
    ...
    manageUsersButton.addEventListener('click', () => {
        ...
        document.querySelectorAll('.edit-button').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.getElementById('edit-username-title').textContent = `Edit User: ${btn.closest('tr').children[1].textContent}`;
                showNotification('Edit user modal opened (simulated).', 'info');
            });
        });
        document.querySelectorAll('.delete-button').forEach(btn => {
            btn.addEventListener('click', () => {
                showNotification(`User "${btn.closest('tr').children[1].textContent}" deleted (simulated).`, 'success');
        ...
        document.querySelectorAll('.modal-tab-button').forEach(tabButton => {
            tabButton.addEventListener('click', () => {
                ...
                showNotification(`Switched to "${tabButton.dataset.tab}" tab (simulated).`, 'info');
            });
        });
        ...
        }, { once: true }); // Use once: true to prevent multiple submissions on dummy form
    ...
    manageCustomCommandsButton.addEventListener('click', () => {
        ...
        `;
        document.getElementById('save-commands-button').addEventListener('click', () => {
            showNotification('Commands saved (simulated).', 'success');
        }, { once: true });
        document.getElementById('reset-commands-button').addEventListener('click', () => {
